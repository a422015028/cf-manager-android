...
exports.assertUrlSafe = assertUrlSafe;
...
function isPrivateIp(ip) {
    ...
    if (node_net_1.default.isIPv6(ip)) {
        ...
        if (norm.startsWith('fc') || norm.startsWith('fd'))
            return true; // fc00::/7
        if (/^fe[8-9a-b]/.test(norm))
            return true; // fe80::/10
        if (norm.startsWith('::ffff:'))
            return isPrivateIp(norm.slice('::ffff:'.length));
        return false;
    }
    return false;
}
...
async function ensureHostAllowed(host, allowlist) {
    if (allowlist && !allowlist.includes(host.toLowerCase())) {
        const err = new Error(`URL host "${host}" is not in the allowed deploy-source allowlist`);
        err.statusCode = 403;
        throw err;
    }
    if (node_net_1.default.isIP(host)) {
        if (isPrivateIp(host)) {
            const err = new Error(`URL host "${host}" is a non-public address and is blocked`);
            err.statusCode = 403;
            throw err;
        }
        return;
    ...
    let addresses;
    try {
        addresses = await promises_1.default.lookup(host, { all: true });
    }
    catch (e) {
        const err = new Error(`Failed to resolve host "${host}": ${e.message}`);
        err.statusCode = 400;
        throw err;
    }
    if (addresses.some((a) => isPrivateIp(a.address))) {
        const err = new Error(`URL host "${host}" resolves to a non-public address and is blocked`);
        err.statusCode = 403;
        throw err;
    }
}
...
// 但仍拦截其它私网地址。
async function assertUrlSafe(raw) {
    let url;
    try {
        url = new URL(raw);
    }
    ...
    if (url.protocol !== 'https:' && !(isLocal && url.protocol === 'http:')) {
        const err = new Error('Only HTTPS (or http://localhost for dev) URLs are allowed');
        err.statusCode = 403;
        throw err;
    }
    if (isLocal)
...
async function fetchScriptSafely(raw) {
    ...
    try {
        ...
        const err = new Error('Invalid script URL');
        err.statusCode = 400;
        throw err;
    }
    if (current.protocol !== 'https:') {
        const err = new Error('Only HTTPS script URLs are allowed');
        err.statusCode = 403;
        throw err;
    }
    ...
    while (true) {
        ...
        if (resp.status === 200) {
            ...
            if (!ACCEPTED_SCRIPT_CT.test(ct)) {
                const err = new Error(`Refused script with unsupported Content-Type: ${ct || '(none)'}`);
                err.statusCode = 400;
                throw err;
            }
            const buf = Buffer.from(await resp.arrayBuffer());
            if (buf.byteLength > MAX_SCRIPT_SIZE) {
                const err = new Error('Script exceeds maximum allowed size (5 MiB)');
                err.statusCode = 413;
                throw err;
            }
        ...
        if ([301, 302, 303, 307, 308].includes(resp.status) || resp.status === 0) {
            ...
            if (!loc) {
                const err = new Error(`Redirect from ${current.toString()} has no Location`);
                err.statusCode = 400;
                throw err;
            }
            hops += 1;
            ...
            try {
                ...
                const err = new Error(`Invalid redirect location: ${loc}`);
                err.statusCode = 400;
                throw err;
            }
            if (next.protocol !== 'https:') {
                const err = new Error('Redirect to non-HTTPS URL is blocked');
                err.statusCode = 403;
                throw err;
            }
        ...
        const err = new Error(`Failed to fetch script: HTTP ${resp.status}`);
        err.statusCode = 400;
        throw err;
    }
}

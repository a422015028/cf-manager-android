...
exports.deployWorkerAssets = deployWorkerAssets;
...
async function deployWorkerAssets(account, scriptName, files) {
    ...
    const sessionResp = await withRetry(() => (0, proxyService_1.proxyFetch)(`${CF_BASE}/accounts/${accountId}/workers/scripts/${scriptName}/assets-upload-session`, {
        method: 'POST',
    ...
    for (let bi = 0; bi < buckets.length; bi++) {
        ...
        for (const hash of bucket) {
            ...
            const ct = (0, staticAssets_1.getContentType)(filePath);
            upForm.append(hash, new Blob([buf.toString('base64')], { type: ct }), hash);
        }
        const upResp = await withRetry(() => (0, proxyService_1.proxyFetch)(`${CF_BASE}/accounts/${accountId}/workers/assets/upload?base64=true`, {
            method: 'POST',

 ...
 * 4. Secrets 覆盖检查
 */
async function preflight(account, template, params) {
    const warnings = [];
    ...
    try {
        const resp = await (0, proxyService_1.proxyFetch)(`${CF_BASE}/accounts/${accountId}/workers/services/${params.name}`, {
        ...
        if (resp.ok) {
            ...
            try {
                const settingsResp = await (0, proxyService_1.proxyFetch)(`${CF_BASE}/accounts/${accountId}/workers/scripts/${params.name}/settings`, {
                    headers: { ...deployHeaders },
                }, 30000, undefined, account);
                if (settingsResp.ok) {
    ...
    const deployPath = workerExists ? 'versions-api' : 'legacy-put';
    ...
    logger_1.appLogger.info(`[Preflight] Worker ${params.name}: exists=${workerExists}, path=${deployPath}, canProceed=${canProceed}`);
    return {
        workerExists,
        deployPath,
        configDiff,
